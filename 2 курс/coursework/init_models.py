...
def get_features(vectorizer, data, if_wv=False, w2v_model=None):
	...
	features_to_remove = topics + ['Unnamed: 0', 'path', 'text']
...
def init_model(classifier_name, vectorizer, path, if_test=False, if_wv=False, w2v_model=None, rs=18):
	df = get_df(path)
	global topics
	...
	if if_test:
		train_data, test_data = train_test_split(df[df['if_topic'] == 1], test_size=0.1, random_state=rs)
		predict_data = df[df['if_topic'] == 0]
	else:
	...
	if if_test:
		model_accuracy, predictions_test_df = predict(vectorizer, classifier, test_data, if_wv=if_wv, w2v_model=w2v_model) 
		predictions_df = predict(vectorizer, classifier, predict_data, if_test, if_wv=if_wv, w2v_model=w2v_model)
		test_df_with_predictions = pd.concat([predict_data,predictions_df.set_index(test_data.index)], axis=1)
	else:
